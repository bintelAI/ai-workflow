...
export const runSimulationLogic = async (
        ...
        const conditions = (group.conditions || []).map((cond: any) => {
          ...
          switch (operator) {
            ...
            case '<=':
              return `${safeVariable} <= ${val}`
            case 'contains':
              return `String(${safeVariable}).includes(String(${val}))`
            case 'not_contains':
              return `!String(${safeVariable}).includes(String(${val}))`
            case 'empty':
              return `!${safeVariable} || (typeof ${safeVariable} === 'string' && ${safeVariable}.length === 0)`
            case 'not_empty':
              return `${safeVariable} && (typeof ${safeVariable} === 'string' ? ${safeVariable}.length > 0 : true)`
            default:
              return 'true'
          }
        })
  ...
  if (customInput) {
    try {
      initialPayload = JSON.parse(customInput)
    } catch (e) {}
  ...
  while (queue.length > 0 && steps < MAX_STEPS) {
    ...
    if (node.type === WorkflowNodeType.LOOP) {
          ...
          if (mode === 'iteration') {
              ...
              const chunkPromises = chunk.map(async (item, chunkIndex) => {
                ...
                childNodes.forEach(child => {
                  ...
                  if (child.type === WorkflowNodeType.LLM) {
                    mockOutput = {
                      text: `AI 处理了第 ${actualIndex + 1} 项数据: ${JSON.stringify(item)}`,
                      response: { id: `iter-${actualIndex}` },
                    }
            ...
            for (let i = 0; i < arrayData.length; i++) {
              ...
              if (config.terminationConditions && config.terminationConditions.length > 0) {
                const shouldTerminate = evaluateConditions(
              ...
              childNodes.forEach(child => {
                ...
                if (child.type === WorkflowNodeType.LLM) {
                  mockOutput = {
                    text: `AI 顺序处理了第 ${i + 1} 项数据: ${JSON.stringify(item)}`,
                    response: { id: `loop-${i}` },
                  }
                } else if (child.type === WorkflowNodeType.API_CALL) {
                  mockOutput = { data: { result: 'success', item: item }, status: 200 }
      ...
      const enabledQueryParams = normalizeArray(config.queryParams).filter(
        (param: any) => param.enabled
      )
      ...
      // Process headers
      const enabledHeaders = normalizeArray(config.headers).filter(
        (header: any) => header.enabled && header.key && header.key.trim() !== ''
      )
