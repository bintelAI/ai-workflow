...
const App: React.FC = () => {
  const { validateWorkflow, toggleDrawer, runSimulation, toggleSettings, categories, activeCategoryId } = useWorkflowStore();
    ...
    <div className="h-screen w-screen flex flex-col overflow-hidden bg-slate-50 text-slate-900 font-sans">
      ...
      <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-6 z-10 shadow-sm shrink-0">
        <div className="flex items-center gap-3">
            ...
            <div>
                <h1 className="font-bold text-slate-800 text-lg leading-tight flex items-center gap-2">
                    FlowMaster 
                    <span className="text-[10px] bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded-full font-bold">AI Pro</span>
                </h1>
                <p className="text-[10px] text-slate-400 font-medium">当前模式: {activeCategoryName}</p>
            </div>
        </div>
        ...
        <div className="flex items-center gap-3">
             ...
             <button onClick={handleVerify} className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 rounded-md transition-colors">
