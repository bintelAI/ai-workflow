...
const getNodeIcon = (type: string) => {
  switch (type) {
    case WorkflowNodeType.START: return <PlayCircle className="w-5 h-5 text-emerald-500" />;
    case WorkflowNodeType.END: return <StopCircle className="w-5 h-5 text-rose-500" />;
    case WorkflowNodeType.APPROVAL: return <CheckSquare className="w-5 h-5 text-blue-500" />;
    case WorkflowNodeType.CC: return <Send className="w-5 h-5 text-indigo-500" />;
    case WorkflowNodeType.CONDITION: return <GitFork className="w-5 h-5 text-amber-500" />;
    case WorkflowNodeType.PARALLEL: return <GitMerge className="w-5 h-5 text-teal-500" />;
    case WorkflowNodeType.API_CALL: return <Globe className="w-5 h-5 text-violet-500" />;
    case WorkflowNodeType.NOTIFICATION: return <Bell className="w-5 h-5 text-orange-500" />;
    case WorkflowNodeType.DELAY: return <Clock className="w-5 h-5 text-yellow-500" />;
    case WorkflowNodeType.DATA_OP: return <Database className="w-5 h-5 text-cyan-500" />;
...
const getNodeTypeLabel = (type: string) => {
  switch (type) {
    ...
    case WorkflowNodeType.CONDITION: return '条件节点';
    case WorkflowNodeType.PARALLEL: return '并行节点';
...
const getNodeColor = (type: string, selected: boolean, executionStatus?: string) => {
  ...
  if (executionStatus === 'success') return 'ring-2 ring-emerald-500 border-emerald-500 shadow-md shadow-emerald-100 bg-emerald-50/10';
  if (executionStatus === 'failed') return 'ring-2 ring-rose-500 border-rose-500 shadow-md shadow-rose-100 bg-rose-50/10';
