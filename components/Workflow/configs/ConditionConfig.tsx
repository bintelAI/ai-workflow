...
export const ConditionConfig: React.FC<ConditionConfigProps> = ({ config, onConfigChange }) => {
  const [mode, setMode] = useState<'builder' | 'manual'>(
    config?.conditionGroups ? 'builder' : 'manual'
  )
  ...
  return (
    <div className="space-y-4">
      <div className="bg-amber-50 p-3 rounded-lg border border-amber-100 text-xs text-amber-700 flex items-start gap-2">
        ...
        <p>
          若表达式结果为 Truthy，则执行“是”分支；否则执行“否”分支。您可以使用可视化编辑器或直接编写
          JavaScript。
        </p>
      </div>
      ...
      {mode === 'builder' ? (
        <ConditionBuilder
          ...
          onChange={groups =>
            onConfigChange('conditionGroups', groups.length > 0 ? groups : undefined)
          }
        />
      ) : (
