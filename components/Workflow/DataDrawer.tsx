...
export const DataDrawer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (startNode?.data.config && (startNode.data.config as any)?.devInput) {
      setInputJson((startNode.data.config as any).devInput)
    }
  }, [nodes, isDrawerOpen])
  ...
  return (
    <div
      className={`fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] transition-all duration-300 ease-in-out z-30 flex flex-col ${isDrawerOpen ? 'h-[400px]' : 'h-0'}`}
    >
      {/* Header */}
      <div className="h-12 border-b border-slate-200 flex items-center justify-between px-4 bg-slate-50 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="p-1.5 hover:bg-slate-200 rounded-md text-slate-500 ml-2"
          >
            <X size={18} />
          </button>
        </div>
              ...
              {simulationLogs.length === 0 ? (
                ...
                <div className="relative pl-3 space-y-2 before:content-[''] before:absolute before:left-[19px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
                  {simulationLogs.map(log => (
                    ...
                    >
                      <div
                        className={`absolute left-[5px] top-3.5 w-2.5 h-2.5 rounded-full border-2 bg-white z-10 ${log.status === 'success' ? 'border-emerald-500' : 'border-red-500'}`}
                      ></div>
                      <div className="flex items-center justify-between mb-0.5">
                        <span
                          className={`font-medium text-xs truncate max-w-[120px] ${log.status === 'failed' ? 'text-red-600' : 'text-slate-800'}`}
                        >
                          {log.nodeLabel}
                        </span>
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-slate-500">
                        <span
                          className={`flex items-center gap-0.5 ${log.status === 'success' ? 'text-emerald-600' : 'text-red-600'}`}
                        >
                          {log.status === 'success' ? (
              ...
              {selectedLog ? (
                <>
                  ...
                  <div className="bg-white rounded-lg border border-slate-200 shadow-sm flex-1 flex flex-col overflow-hidden h-full">
                    ...
                    <div className="flex-1 p-3 overflow-auto">
                      <pre
                        className={`text-xs font-mono ${selectedLog.status === 'failed' ? 'text-red-600' : 'text-slate-600'}`}
                      >
                        {selectedLog.errorMessage
                          ? JSON.stringify(
                              { error: selectedLog.errorMessage, ...selectedLog.output },
                              null,
                              2
                            )
          ...
          <div className="flex-1 flex gap-4 p-4 bg-slate-50 overflow-hidden">
            <div className="w-64 shrink-0 pt-2">
              ...
              <div className="text-xs text-slate-500 space-y-2 leading-relaxed">
                ...
                <p>
                  点击上方 <span className="font-bold text-indigo-600">重新模拟</span> 后，该 JSON
                  将作为 <code>payload</code> 传递给流程中的所有节点。
                </p>
        ...
        {activeTab === 'ai' && (
          ...
          <div className="flex-1 p-6 bg-slate-50 overflow-y-auto">
            <div className="max-w-4xl mx-auto space-y-6">
              {/* Summary Card */}
              <div
                className={`bg-white rounded-xl p-4 shadow-sm border flex gap-4 ${failedLogs.length > 0 ? 'border-red-100' : 'border-slate-200'}`}
              >
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center text-white shadow-lg ${failedLogs.length > 0 ? 'bg-gradient-to-br from-red-500 to-orange-600 shadow-red-200' : 'bg-gradient-to-br from-emerald-500 to-teal-600 shadow-emerald-200'}`}
                >
                ...
                <div className="bg-white rounded-xl p-4 shadow-sm border border-red-100 animate-in fade-in slide-in-from-bottom-2">
                  ...
                  <div className="space-y-3">
                    {failedLogs.map((log, i) => (
                      <div
                        key={i}
                        className="flex flex-col gap-2 p-3 bg-red-50 rounded-lg border border-red-100"
                      >
                        ...
                        <div className="ml-3.5 flex items-start gap-1.5">
                          ...
                          <span className="text-xs text-slate-600">
                            <strong>AI 建议：</strong>
                            {log.nodeType === 'api_call' &&
                              '检查目标 API URL 是否可访问，或增加超时重试配置。'}
                            {log.nodeType === 'approval' &&
                              '检查审批条件设置，确保金额或参数未超出自动处理阈值。'}
                            {!['api_call', 'approval'].includes(log.nodeType) &&
                              '检查该节点的输入参数映射是否正确。'}
                          </span>
                        </div>
                      </div>
                      ...
                      .map((l, i) => (
                        ...
                        >
                          <div className="flex gap-2">
                            ...
                            <span>
                              节点 "{l.nodeLabel}" 耗时较长 ({l.duration}ms)。
                            </span>
                          </div>
                        </li>
                      ))}
