...
const NodeAddMenu = () => {
  const { 
    edgeMenu, closeEdgeMenu, insertNodeBetween,
    nodeMenu, closeNodeMenu, appendNode,
    ...
    <div className="flex-1 h-full w-full bg-slate-50 relative" ref={reactFlowWrapper}>
      ...
      >
        ...
        <MiniMap 
            nodeColor={(node) => {
                switch (node.type) {
                    case WorkflowNodeType.START: return '#10b981';
                    case WorkflowNodeType.END: return '#f43f5e';
                    case WorkflowNodeType.LOOP: return '#6366f1';
        ...
        />
        <Panel position="top-right" className="bg-white/80 backdrop-blur-sm p-2 rounded-lg border border-slate-200 shadow-sm text-xs text-slate-500">
